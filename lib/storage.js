...
UserDatabase.prototype = {
  ...
  find: function(nick) {
    ...
    console.log("looking for", nick);
    for (var i = 0; i < this.registered.length; i++) {
      var user = this.registered[i];
      console.log(" user:", user && user.nick, this.server.normalizeName(user.nick) === nick)
      if (user && this.server.normalizeName(user.nick) === nick) {
        return user;
      }
    }
  },
